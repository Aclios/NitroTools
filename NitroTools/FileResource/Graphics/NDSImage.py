...
class NDSImage:
    ...
    def generate_tile_list(self):
        ...
        tiles = [
            Tile(
                data[tile_datasize * idx : tile_datasize * (idx + 1)],
                self.bit_depth,
                self.linear,
            )
    ...
    def build_hor_image(self, pal_idx: int):
        ...
        for j in range(shape_height_count):
            for i in range(shape_width_count):
                shape = Shape(
                    tiles[tile_idx : tile_idx + tile_count_par_shape],
                    self.shape_size,
                    pal_idx,
                    self.bit_depth,
                    False,
    ...
    def build_cells(self):
        ...
        for cell_bank in self.Cell.cebk.cell_banks:
            ...
            max_x = max(
                [
                    OAM_data.x_pos + OAM_data.shape_size[0]
                    for OAM_data in cell_bank.OAM_data_list
                ]
            )
            max_y = max(
                [
                    OAM_data.y_pos + OAM_data.shape_size[1]
                    for OAM_data in cell_bank.OAM_data_list
                ]
            )
            ...
            shape = Shape(
                im.crop(
                    (
                        ...
                        OAM_data.x_pos + OAM_data.shape.width - min_x,
                        OAM_data.y_pos + OAM_data.shape.height - min_y,
                    )
                ),
                OAM_data.shape_size,
                OAM_data.pal_idx,
                ...
                shape = Shape(
                    im.crop(
                        (
                            i * self.shape_width,
                            j * self.shape_height,
                            (i + 1) * self.shape_width,
                            (j + 1) * self.shape_height,
                        )
                    ),
                    self.shape_size,
                    0,
